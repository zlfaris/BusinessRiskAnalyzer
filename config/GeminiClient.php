...
class GeminiClient
{
    ...
    {
        // 1. Robustly fetch API key from multiple sources as fallback
        $rawKey = getenv('GEMINI_API_KEY');
        if (empty($rawKey)) {
            $rawKey = $_ENV['GEMINI_API_KEY'] ?? ($_SERVER['GEMINI_API_KEY'] ?? '');
        }

        // 2. Strict defensive trimming
        ...
        if (empty($this->apiKey)) {
            throw new RuntimeException("Gemini API Key is completely missing from the environment configuration.");
        }
    ...
    public function analyzeRisk(array $businessData): array
    {
        ...
        $options = [
            'http' => [
                ...
                'ignore_errors' => true // Allows capturing response body on HTTP errors
            ]
        ];
        ...
        if ($response === false) {
            ...
            throw new RuntimeException("HTTP Request Error: " . ($error['message'] ?? 'Unknown error'));
        }
        ...
        throw new RuntimeException("Failed to parse a valid JSON response from Gemini API.");
    }
